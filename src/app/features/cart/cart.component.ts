    ...
    <div class="min-h-screen bg-gray-50 py-6">
      <div class="max-w-3xl mx-auto px-4">
        <div class="flex items-center justify-between mb-6">
          ...
          <a routerLink="/catalog" class="text-sm text-rose-500 hover:text-rose-600 font-semibold transition-colors">
            ← Seguir comprando
          </a>
        </div>
        ...
        @if (cartService.items().length === 0) {
          <div class="bg-white rounded-2xl shadow-sm p-12 text-center">
            ...
            <p class="text-gray-400 text-sm mb-6">Agrega productos desde el catálogo para comenzar.</p>
            <a
              routerLink="/catalog"
            ...
            @for (item of cartService.items(); track item.product.id) {
              <div class="bg-white rounded-2xl shadow-sm p-4 flex items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                  <p class="text-sm font-semibold text-gray-800 line-clamp-1">{{ item.product.title }}</p>
                  <p class="text-xs text-gray-400 capitalize mt-0.5">{{ item.product.category }}</p>
                  <p class="text-sm font-bold text-rose-500 mt-1">{{'$'}}{{ item.product.price.toFixed(2) }}</p>
                </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm p-6">
            ...
            <div class="border-t border-gray-100 pt-4 flex justify-between items-center mb-5">
              ...
              <span class="text-2xl font-extrabold text-gray-900">{{'$'}}{{ cartService.totalPrice().toFixed(2) }}</span>
            </div>
