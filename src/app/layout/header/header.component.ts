    ...
    <header class="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm">
      <div class="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        <a routerLink="/catalog" class="flex items-center gap-2 select-none">
          <span
            class="text-2xl font-extrabold bg-gradient-to-r from-rose-500 to-pink-500 bg-clip-text text-transparent tracking-tight"
          >
            NYX
          </span>
        ...
        >
          ...
          @if (cartService.totalItems() > 0) {
            <span
              class="absolute -top-1.5 -right-1.5 bg-rose-500 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center leading-none"
            >
